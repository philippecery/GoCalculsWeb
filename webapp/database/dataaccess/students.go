...
func GetAllStudents() []*document.Student {
	...
	if cursor, err = collection.Users.Find(context.TODO(), bson.M{"status": user.Enabled, "role": user.Student}); err != nil {
...
func GetStudentByID(id string) *document.Student {
	student := new(document.Student)
	if err := collection.Users.FindOne(context.TODO(), bson.M{"userid": id, "role": user.Student}).Decode(student); err != nil {
...
func SetGradeForStudents(gradeID string, students []string) error {
	if _, err := collection.Users.UpdateMany(context.TODO(), bson.M{"role": user.Student, "userid": bson.M{"$in": students}}, bson.M{"$set": bson.M{"gradeid": gradeID}}); err != nil {
...
func UnassignGradeForStudent(gradeID, studentID string) error {
	if _, err := collection.Users.UpdateOne(context.TODO(), bson.M{"role": user.Student, "userid": studentID, "gradeid": gradeID}, bson.M{"$set": bson.M{"gradeid": ""}}); err != nil {
...
func AssignGradeForStudent(gradeID, studentID string) error {
	if _, err := collection.Users.UpdateOne(context.TODO(), bson.M{"role": user.Student, "userid": studentID}, bson.M{"$set": bson.M{"gradeid": gradeID}}); err != nil {
